...
export default function SignInPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if ("errors" in result) {
      ...
      setLoading(false); // encerra spinner em erro
    } else {
      setError("");
      setErrors([]);
      setLoading(false); // encerra spinner em sucesso
      router.push("/home");
    }
  }
        ...
        <div className="flex-1 p-10 flex flex-col justify-center">
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <button
              ...
              disabled={loading}
              aria-busy={loading}
              className="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-indigo-600 text-white disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading && (
                <svg
